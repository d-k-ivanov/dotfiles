...
autoenv_check_authz_and_run()
{
    local _envfile
    _envfile="${1}"
    if autoenv_check_authz "${_envfile}"
    then
        autoenv_source "${_envfile}"
        return 0
    fi

    if [ -n "${AUTOENV_ASSUME_YES}" ]
    then # Don't ask for permission if "assume yes" is switched on
...
if setopt 2> /dev/null | grep -q aliasfuncdef
then
...
then
    ...
    enable_autoenv
elif command -v sha1sum 2>/dev/null >&2
then
    autoenv_shasum()
    {
        sha1sum "${@}"
    ...
    enable_autoenv
elif command -v shasum 2>/dev/null >&2
then
    autoenv_shasum() {
        shasum "${@}"
    }
