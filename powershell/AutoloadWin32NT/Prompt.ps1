...
function devops
{
    ...
    Set-Item -Path Env:DEVOPSPROMPT -Value "1"
}

function undevops
{
    Remove-Item Env:DEVOPSPROMPT -ErrorAction SilentlyContinue
    Set-Promt
}

if ($Env:DEVOPSPROMPT -and -not $Env:IS_IN_VSCODE -and (Get-Command starship -ErrorAction SilentlyContinue | Test-Path))
{
    devops
}
else
